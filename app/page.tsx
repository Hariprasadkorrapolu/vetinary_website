          ...
          <div className="grid items-center gap-12 lg:grid-cols-2">
            ...
            <div>
              ...
              <p className="mt-6 text-lg leading-8 text-slate-600">
                ...
                segment, producing and marketing a wide range of products for livestock, poultry,
                and companion animal segments.
              </p>
              <p className="mt-5 text-lg leading-8 text-slate-600">
                With the motto <span className="font-semibold text-ink">“Every Farm Care We Are There”</span>,
                everything we do reflects our commitment to total animal wellness through quality,
                effective, and affordable animal health care solutions.
              </p>
              <div className="mt-7 grid gap-4 sm:grid-cols-2">
                {["Product innovation", "Quality assurance", "Flexible solutions", "Cost-effectiveness"].map(
                  (item) => (
                    <p key={item} className="flex items-center gap-3 text-sm font-semibold text-ink">
        ...
        <Container className="mt-12">
          <div className="no-scrollbar flex snap-x gap-6 overflow-x-auto pb-4 lg:grid lg:grid-cols-4 lg:overflow-visible">
            {featuredProducts.slice(0, 4).map((product) => (
              <div key={product.id} className="min-w-[82%] snap-start sm:min-w-[45%] lg:min-w-0">
      ...
      <section className="section-pad bg-mist">
        ...
        <Container className="mt-12">
          <div className="no-scrollbar flex gap-6 overflow-x-auto pb-3 lg:grid lg:grid-cols-3 lg:overflow-visible">
            {reviews.map((review) => (
              <article key={review.name} className="min-w-[86%] rounded-[1.75rem] bg-white p-6 shadow-soft sm:min-w-[44%] lg:min-w-0">
